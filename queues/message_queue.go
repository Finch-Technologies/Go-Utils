...
func Dequeue[T interface{}](ctx context.Context, queue Queue, options ...types.DequeueOptions) (T, error) {
	...
	if mq == nil {
		return payload, fmt.Errorf("no message queue driver found")
	}

	itemStr, err := mq.Dequeue(ctx, string(queue))

	if err != nil {
		return payload, fmt.Errorf("failed to dequeue item from queue: %s", err)
	}
	...
	err = json.Unmarshal([]byte(itemStr), &payload)

	if err != nil {
		return payload, fmt.Errorf("failed to unmarshal payload from json: %s", err)
	}
